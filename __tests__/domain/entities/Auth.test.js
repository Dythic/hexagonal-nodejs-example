...
describe("Auth Entity", () => {
  ...
  describe("create", () => {
    ...
    it("devrait rejeter un userId manquant", () => {
      expect(() => Auth.create("", validEmail, validHashedPassword)).toThrow(
        "UserId, email et mot de passe hashé sont requis",
      );
      expect(() => Auth.create(null, validEmail, validHashedPassword)).toThrow(
        "UserId, email et mot de passe hashé sont requis",
      );
    });

    it("devrait rejeter un email manquant", () => {
      expect(() => Auth.create(validUserId, "", validHashedPassword)).toThrow(
        "UserId, email et mot de passe hashé sont requis",
      );
      expect(() => Auth.create(validUserId, null, validHashedPassword)).toThrow(
        "UserId, email et mot de passe hashé sont requis",
      );
    });

    it("devrait rejeter un mot de passe hashé manquant", () => {
      expect(() => Auth.create(validUserId, validEmail, "")).toThrow(
        "UserId, email et mot de passe hashé sont requis",
      );
      expect(() => Auth.create(validUserId, validEmail, null)).toThrow(
        "UserId, email et mot de passe hashé sont requis",
      );
    });
    ...
    it("devrait rejeter un rôle invalide", () => {
      ...
      expect(() =>
        Auth.create(validUserId, validEmail, validHashedPassword, "user"),
      ).toThrow("Rôle invalide");
    });
  });
