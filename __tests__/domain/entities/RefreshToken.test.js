...
describe("RefreshToken Entity", () => {
  ...
  describe("create", () => {
    ...
    it("devrait rejeter un userId manquant", () => {
      expect(() => RefreshToken.create("", validToken, futureDate)).toThrow(
        "UserId, token et date d'expiration sont requis",
      );
      expect(() => RefreshToken.create(null, validToken, futureDate)).toThrow(
        "UserId, token et date d'expiration sont requis",
      );
    });

    it("devrait rejeter un token manquant", () => {
      expect(() => RefreshToken.create(validUserId, "", futureDate)).toThrow(
        "UserId, token et date d'expiration sont requis",
      );
      expect(() => RefreshToken.create(validUserId, null, futureDate)).toThrow(
        "UserId, token et date d'expiration sont requis",
      );
    });

    it("devrait rejeter une date d'expiration manquante", () => {
      expect(() => RefreshToken.create(validUserId, validToken, null)).toThrow(
        "UserId, token et date d'expiration sont requis",
      );
      expect(() =>
        RefreshToken.create(validUserId, validToken, undefined),
