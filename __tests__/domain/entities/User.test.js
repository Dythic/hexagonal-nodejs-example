...
describe('User Entity', () => {
  describe('création d\'utilisateur', () => {
    ...
    it('devrait rejeter un email manquant', () => {
      expect(() => User.create('', 'John Doe')).toThrow('Email et nom sont requis');
      expect(() => User.create(null, 'John Doe')).toThrow('Email et nom sont requis');
    });

    it('devrait rejeter un nom manquant', () => {
      expect(() => User.create('test@example.com', '')).toThrow('Email et nom sont requis');
      expect(() => User.create('test@example.com', null)).toThrow('Email et nom sont requis');
    });

    it('devrait rejeter un email invalide', () => {
      expect(() => User.create('invalid-email', 'John Doe')).toThrow('Format d\'email invalide');
      expect(() => User.create('test@', 'John Doe')).toThrow('Format d\'email invalide');
    });

    it('devrait rejeter un nom trop court', () => {
      expect(() => User.create('test@example.com', 'A')).toThrow('Le nom doit contenir au moins 2 caractères');
    });
  });
