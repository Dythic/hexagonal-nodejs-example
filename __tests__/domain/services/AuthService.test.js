...
describe('AuthService', () => {
    ...
    describe('register', () => {
        ...
        it('devrait rejeter si l\'utilisateur existe déjà', async () => {
            const existingAuth = new Auth('user123', 'test@example.com', 'hashedPassword', 'USER');
            mockAuthRepository.findByEmail.mockResolvedValue(existingAuth);

            await expect(authService.register('test@example.com', 'John', 'password123'))
    ...
    describe('login', () => {
        ...
        it('devrait rejeter avec un mauvais mot de passe', async () => {
            const auth = new Auth('user123', 'test@example.com', 'hashedPassword', 'USER');
