...
describe('BcryptPasswordService', () => {
  ...
  describe('hash', () => {
    ...
    it('devrait rejeter un mot de passe vide', async () => {
      await expect(passwordService.hash('')).rejects.toThrow('Le mot de passe doit contenir au moins 6 caractères');
    });

    it('devrait rejeter un mot de passe null', async () => {
      await expect(passwordService.hash(null)).rejects.toThrow('Le mot de passe doit contenir au moins 6 caractères');
    });

    it('devrait rejeter un mot de passe undefined', async () => {
      await expect(passwordService.hash(undefined)).rejects.toThrow('Le mot de passe doit contenir au moins 6 caractères');
    });

    it('devrait rejeter un mot de passe trop court', async () => {
      await expect(passwordService.hash('12345')).rejects.toThrow('Le mot de passe doit contenir au moins 6 caractères');
    });
  ...
  describe('compare', () => {
    ...
    it('devrait être sensible à la casse', async () => {
      ...
      const isValidLower = await passwordService.compare('password123', hashedPassword);
      const isValidUpper = await passwordService.compare('PASSWORD123', hashedPassword);
      const isValidCorrect = await passwordService.compare('Password123', hashedPassword);
