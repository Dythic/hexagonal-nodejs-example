...
describe('JwtTokenService', () => {
  ...
  describe('constructor', () => {
    ...
    it('devrait lancer une erreur si JWT_SECRET manque', () => {
      ...
      expect(() => new JwtTokenService()).toThrow('JWT secrets must be configured');
    });
  ...
  describe('verifyAccessToken', () => {
    ...
    it('devrait rejeter un token expiré', () => {
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          expect(() => shortLivedService.verifyAccessToken(token))
            .toThrow('Token d\'accès invalide');
          resolve();
        }, 10);
      });
    ...
    it('devrait rejeter un token avec mauvais issuer', () => {
      ...
      expect(() => tokenService.verifyAccessToken(token))
        .toThrow('Token d\'accès invalide');
    });
  ...
  describe('verifyRefreshToken', () => {
    ...
    it('devrait rejeter un refresh token invalide', () => {
      ...
      expect(() => tokenService.verifyRefreshToken(invalidToken))
        .toThrow('Refresh token invalide');
    });
    ...
    it('devrait rejeter un access token utilisé comme refresh token', () => {
      ...
      expect(() => tokenService.verifyRefreshToken(accessToken))
        .toThrow('Refresh token invalide');
    });
  });
