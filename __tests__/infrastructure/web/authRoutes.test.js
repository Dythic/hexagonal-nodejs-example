...
describe('AuthRoutes', () => {
  ...
  beforeEach(() => {
    mockAuthController = {
      ...
      refreshToken: jest.fn((req, res) => res.json({ success: true, tokens: {} })),
      logout: jest.fn((req, res) => res.json({ success: true, message: 'Logged out' })),
      getProfile: jest.fn((req, res) => res.json({ success: true, data: { user: req.user } })),
      changePassword: jest.fn((req, res) => res.json({ success: true, message: 'Password changed' }))
    };
